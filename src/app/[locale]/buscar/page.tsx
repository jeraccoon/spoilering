...
async function getInitialResults(filter: Filter, locale: string): Promise<Result[]> {
  ...
  const { data } = await q
  return ((data ?? []) as Result[]).map((w) => localizeWork(w, locale))
}
...
export default async function BuscarPage({ params, searchParams }: Props) {
  ...
  const initialResults = await getInitialResults(filter, locale)
