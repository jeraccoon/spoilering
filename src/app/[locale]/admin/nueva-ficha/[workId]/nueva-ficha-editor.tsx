...
export function NuevaFichaEditor({ work }: { work: Work }) {
  const t = useTranslations('Admin.newCard')
  ...
  async function handleDiscard() {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json()
        setError(data.error ?? t('errors.discard'))
        setDiscarding(false)
        return
      }
  ...
  const workTypeLabel = work.type === 'movie' || work.type === 'series' || work.type === 'book'
    ? tw(work.type)
    : work.type
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      <div className="mb-2 text-sm text-ink/55">
        <Link href="/admin" className="hover:text-ink">{t('breadcrumbAdmin')}</Link>
        <span className="mx-1.5">/</span>
        <Link href="/admin/nueva-obra" className="hover:text-ink">{t('breadcrumbNewWork')}</Link>
        <span className="mx-1.5">/</span>
        <span>{t('breadcrumbConfirm')}</span>
      </div>

      <h1 className="mb-6 text-2xl font-black tracking-tight text-ink">{t('title')}</h1>
